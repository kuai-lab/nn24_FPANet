...
# and https://github.com/mayorx/matlab_ssim_pytorch_implementation/blob/main/calc_ssim.py
...
from metric.metric_util import reorder_image, to_y_channel
...
def calculate_lpips(img1, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    ...
    img1 = reorder_image(img1, input_order=input_order)
    img2 = reorder_image(img2, input_order=input_order)
    img1 = tensor2img(img1).astype(np.float32)
    img2 = tensor2img(img2).astype(np.float32)
    if crop_border != 0:
        img1 = img1[crop_border:-crop_border, crop_border:-crop_border, ...]
        img2 = img2[crop_border:-crop_border, crop_border:-crop_border, ...]
    ...
    loss_fn_vgg = lpips.LPIPS(net='alex', verbose=False).to(DEVICE)  # RGB, normalized to [-1,1]

    mean = [0.5, 0.5, 0.5]
    std = [0.5, 0.5, 0.5]

    img_gt = img2 / 255.
    img_restored = img1 / 255.
